...
import {
  db, uid, allIndustries, getIndustryFull, hydrateIndustry,
  hydrateAgent, hydrateRun, notify, publicUser, seedIfEmpty,
} from './db.js'
import {
  signToken, hash, check, attachUser, requireAuth, requireRole, ownsIndustry, ownsRun,
} from './auth.js'
import { classifyBuffer, hasVision, modelName } from './vision.js'
...
app.get('/api/health', (_req, res) =>
  ok(res, {
    ok: true,
    vision: hasVision ? modelName : null,
...
app.get('/api/runs/:id/route', requireRole('delivery'), ownsRun, async (req, res) => {
  try {
    const run = hydrateRun(db.prepare('select * from runs where id = ?').get(req.params.id))
    const points = [
      run.agent,
      ...run.stops.map((s) => s.pickup),
      run.industry,
    ].filter((p) => Number.isFinite(Number(p?.lat)) && Number.isFinite(Number(p?.lng)))
    if (points.length < 2) throw Object.assign(new Error('This run does not have enough locations to route.'), { status: 400 })
    ...
    if (!route) throw new Error('No drivable route was found for this run.')
    ...
    ok(res, {
      ...
      steps: route.legs.flatMap((leg) => leg.steps.map((step) => ({
        ...
        duration_min: Math.max(1, Math.round(step.duration / 60)),
        instruction: routeInstruction(step.maneuver, step.name),
      }))),
    })
  } catch (e) {
    fail(res, Object.assign(e, { status: e.status || 502 }))
  }
})
...
function routeInstruction(maneuver, street) {
  const type = maneuver?.type
  const modifier = maneuver?.modifier
  const road = street ? ` onto ${street}` : ''
  ...
  if (type === 'new name' || type === 'continue') return `Continue straight${road}`
  if (type === 'turn') return `${modifier ? `Turn ${modifier}` : 'Continue'}${road}`
  return `${type || 'Continue'}${road}`
}
...
app.listen(PORT, () => {
  ...
  console.log(`[api] vision: ${hasVision ? modelName : 'not configured (GEMINI_API_KEY unset)'}`)
