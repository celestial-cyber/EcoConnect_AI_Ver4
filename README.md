...
real OpenStreetMap maps, a live camera, and Gemini vision called from the server.
...
No configuration is required. On first boot the server creates an empty
`data/ecoconnect.db`. Create your own account from the sign-up screen; all
profiles and operational data are stored in that local database.
...
browser ─┬─ React SPA (Vite)  ─── fetch /api ──►  Express  ─┬─ SQLite  (better-sqlite3)
         ├─ getUserMedia (camera)                           ├─ Gemini  (key server-side only)
...
**Vision.** Gemini, called from the server so the key is never in the bundle. Structured JSON
against a schema, low temperature, and an instruction to answer `unclear` rather than name a
plausible object it cannot see. The server clamps every field before it can reach the database.

Set `GEMINI_API_KEY` in the server environment to enable real image classification. Set
`VISION_PROVIDER=gemini` explicitly when both provider keys may be inherited by the server.
With neither key set, it does not guess: it says so and asks you to describe the item. An earlier build guessed from a fixed list and reported a photo of paper as
"Wooden chair" at 89% confidence — a confident wrong answer is worse than no answer.
...
| Variable | Effect |
|---|---|
| `OPENAI_API_KEY` | Turns on OpenAI image classification when `VISION_PROVIDER=openai`. Server-side only. |
| `OPENAI_MODEL` | OpenAI vision model; defaults to `gpt-4o-mini`. |
| `GEMINI_API_KEY` | Gemini image-classification provider. Server-side only. |
| `GEMINI_MODEL` | Default `gemini-3.7-flash`. `gemini-2.0-flash` has been shut down. |
| `VISION_PROVIDER` | Explicit provider selection: `gemini` or `openai`. Gemini is preferred when both keys exist. |
...
`JWT_SECRET` and `OPENAI_API_KEY`, and serve it over HTTPS. The camera and GPS require HTTPS
outside localhost; reverse geocoding uses OpenStreetMap and does not require an AI key.
