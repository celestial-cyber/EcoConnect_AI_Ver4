...
export function DeliveryRunDetail() {
  ...
  useEffect(() => {
    if (!run?.id) return
    db.runRoute(run.id).then(setRouteInfo).catch(() => setRouteInfo(null))
  }, [run?.id])
  ...
  const route = routeInfo?.geometry?.length > 1 ? routeInfo.geometry : [
        ...
        <Card title="Route" hint="Ordered nearest-first, then improved with 2-opt.">
          ...
          {routeInfo && (
            <div style={{ marginTop: 14 }}>
              <div className="kv"><span>ROAD ROUTE</span><b>{routeInfo.distance_km} km · {routeInfo.duration_min} min</b></div>
              <div className="mono" style={{ marginTop: 12, fontSize: 9.5, letterSpacing: '.16em', color: 'var(--muted)' }}>
                TURN-BY-TURN
              </div>
              <div style={{ marginTop: 8 }}>
                {routeInfo.steps.map((step, i) => (
                  <div key={`${step.instruction}-${i}`} style={{ display: 'flex', gap: 10, padding: '9px 0', borderBottom: '1px solid rgba(59,224,127,.08)' }}>
                    <span className="num" style={{ color: 'var(--em2)', width: 24 }}>{String(i + 1).padStart(2, '0')}</span>
                    <span style={{ fontSize: 12.5 }}>{step.instruction} <span style={{ color: 'var(--muted)' }}>· {step.distance_m >= 1000 ? `${(step.distance_m / 1000).toFixed(1)} km` : `${step.distance_m} m`}</span></span>
                  </div>
                ))}
              </div>
            </div>
