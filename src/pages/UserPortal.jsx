...
export function UserScan() {
  ...
  const [item, setItem] = useState({ label: '', type: '', condition: 6, weight: 3 })
  ...
  async function handle(file) {
    ...
    try {
      ...
      setItem({ label: '', type: '', condition: 6, weight: 3 })
    } finally {
      setStep(2)
    }
  }
  ...
  async function confirm() {
    ...
    if (!item.type) {
      toast('Choose a category', 'Select the material so we can match the correct industry.', 'am')
      return
    }
    setBusy(true)
    try {
          ...
          {step === 2 && (
            <>
              ...
              <div style={{ display: 'flex', gap: 10 }}>
                <button className="btn" onClick={confirm} disabled={busy || !geo.coords || !item.type}>
